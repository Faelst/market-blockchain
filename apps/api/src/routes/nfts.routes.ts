...
router.get("/", validateQuery(schemas.nftQuery), (req, res, next) =>
  NftsController.list(req, res).catch(next)
);
router.get("/:id", (req, res, next) =>
  NftsController.get(req, res).catch(next)
);
router.post("/", requireAuth, (req, res, next) =>
  NftsController.mint(req, res).catch(next)
);
